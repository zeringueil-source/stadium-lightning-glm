...
Data source: the public "noaa-nexrad-level2" S3 bucket (AWS Open Data
Program, anonymous/no-key access). Real volume-scan files are large
(10-30MB) and in a specialized binary format, so this script decodes them
with MetPy (metpy.io.Level2File) and only publishes a small, distance-
bounded, decimated extract — NOT the raw file itself.

WHY A SITE LIST, NOT "ALL NEXRAD SITES":
Each site's latest volume scan has to be downloaded (10-30MB) and decoded
every run. Doing that for all ~130+ CONUS sites on a tight schedule would
burn through GitHub Actions' free minutes fast. Instead, this script only
processes the sites listed in sites.txt (one ICAO radar code per line,
e.g. "KOHX") — edit that file (same low-friction GitHub "edit file" flow
you already used for the GLM setup) to change which site(s) get pulled.
The dashboard tells you which site is nearest to whichever venue you pick;
add that code to sites.txt to start getting real data for it.
